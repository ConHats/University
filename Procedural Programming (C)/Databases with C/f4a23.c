...
int main(){
    ...
    {
        printf("Give an ID to insert to the list: ");
        scanf("%d", &Student.ID);
        printf("Give a grade to insest to the list: ");
    ...
    printf("ID with the biggest grade:");
    ...
    if (!EmptyList(List))
    {
        ...
        while (CurrPtr != NilValue)
        {
            if (Node[CurrPtr].Data.grade >= max)
            {
                ...
                Push(Stack, Node[CurrPtr].Data.ID);
            }
...
void printAll(ListPointer List, ListPointer FreePtr, NodeType Node[])
{
    ...
    printf("Storage pool has the following elements\n");
